<template>
  <div class="flex flex-col border-b md:border md:rounded-lg py-4 px-4 bg-white">
    <div class="flex items-start w-full  text-sm md:text-base">
      <img class="w24 h-24 md:w-32 md:h-32 p-2" :src="product.image" alt="cart-image">
      <div class="w-full">
        <div class="line-clamp-2 md:line-clamp-1 font-bold w-full mt-4">{{ product.name }}
        </div>
        <div class="flex items-center mt-2 text-sm" v-if="product.color">
          <div :id="`color-`+product.shop_product_id" class="rounded-full w-4 h-4 me-2 cart-item__color">
            <component is="style">
              #color-{{ product.shop_product_id }} {
        ...
        <div class="flex items-center mt-2 text-sm" v-if="false">
          <font-awesome-icon icon="shield-alt" class="me-2"></font-awesome-icon>
          گارانتی ۱۸ ماهه بازرگانی مهر
        </div>
      </div>
    </div>
    <div class="flex justify-between items-center mt-4">
      <div class="flex items-center ">
        <div class="flex items-center border rounded-md w-max h-8">
          <div class="w-10 flex items-center justify-center text-xl cursor-pointer" @click="minusStockCount">
            <svg class="minus" width="18" height="24" viewBox="0 0 18 18" fill="none"
                 xmlns="http://www.w3.org/2000/svg">
              <path d="M3.95276 8.92395H13.7963" stroke="#9098B1" stroke-width="2" stroke-linecap="round"
                    stroke-linejoin="round"/>
            </svg>
          </div>
          <div class="flex justify-center items-center w-12 h-8">
            {{ cartIsChanging ? '' : this.count }}
            <button class="btn btn-primary btn-circle btn-xs loading" :class="cartIsChanging ? '' : 'hidden'"></button>
          </div>
          <div class="w-10 flex items-center justify-center text-xl cursor-pointer" @click="addStockCount">
            <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M8.81018 4.02454V13.8233" stroke="#9098B1" stroke-width="2" stroke-linecap="round"
                    stroke-linejoin="round"/>
              <path d="M3.88843 8.92395H13.7319" stroke="#9098B1" stroke-width="2" stroke-linecap="round"
                    stroke-linejoin="round"/>
            </svg>
          </div>
        </div>
        <button class="btn btn-outline btn-sm ms-4 border-gray-200 h-8" @click="deleteCart">
          <font-awesome-icon class="me-4 text-gray-500" icon="trash"></font-awesome-icon>
          {{ cartIsChanging ? '' : 'حذف' }}
          <button class="btn btn-primary btn-circle btn-xs loading" :class="cartIsChanging ? '' : 'hidden'"></button>
        </button>
      </div>
      <div class="flex items-center">
        <div class="price me-1">{{ this.$getDelimitedPrice(product.price, product.sale_price, this.count) }}</div>
        <svg class="price" width="14" height="23" viewBox="0 0 14 23" fill="none"
             xmlns="http://www.w3.org/2000/svg">
          <path
    ...
    <div class="text-sm text-error mt-2" :class="[countMinusNoticeShowing? '': 'hidden']">حداقل تعداد قابل خرید برای این
      محصول {{ minOrderCount }}
    </div>
    <div class="text-sm text-error mt-2" :class="[countAddNoticeShowing? '': 'hidden']">حداکثر تعداد قابل خرید برای این
      محصول {{ maxOrderCount }}
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      minOrderCount: this.product.min_order_count,
      maxOrderCount: this.product.max_order_count,
      cartIsChanging: false,
      countAddNoticeShowing: false,
      countMinusNoticeShowing: false
    }
  },
  methods: {
    addStockCount() {
      if (this.count + 1 > this.maxOrderCount) {
        this.countAddNoticeShowing = true
        setTimeout(() => {
          this.countAddNoticeShowing = false
        }, 3000);
      } else if (this.count + 1 > this.product.stock_count) {

      } else {
        if (this.cartIsChanging) {
          return
        }
        this.cartIsChanging = true
        this.$axios.put('customer/cart/' + this.product.cart_id, {count: this.count + 1})
          .then(() => {
            this.count += 1
            window.location.reload()
          }).finally(() => this.cartIsChanging = false)
      }
    },
    minusStockCount() {
      if (this.cartIsChanging) {
      ...
      if (this.count - 1 < this.minOrderCount) {
        this.countMinusNoticeShowing = true
        setTimeout(() => {
          this.countMinusNoticeShowing = false
        }, 3000);
      } else {
        this.cartIsChanging = true
        this.$axios.put('customer/cart/' + this.product.cart_id, {count: this.count - 1})
    ...
    deleteCart() {
      ...
      this.cartIsChanging = true
      this.$axios.delete('customer/cart/' + this.product.cart_id, {count: this.count - 1})
        .then(() => {
          window.location.reload()
        }).finally(() => this.cartIsChanging = false)
    }
