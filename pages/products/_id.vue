<template>
  <div class="flex flex-col mx-auto items-center mt-4">
    ...
    <button class="btn fixed bottom-16 end-4 btn-primary rounded-full md:hidden z-50" @click="addToCart">افزودن به سبد
      خرید
    </button>
    ...
    <div class="container p-4">
      <div class="text-sm breadcrumbs">
        <ul class="text-sm md:text-xs">
          ...
          <li v-for="category in product.categories"><a class="text-gray-700">{{ category.name }}</a></li>
        </ul>
      </div>
    </div>

    <div class="container grid grid-cols-4">
      <ProductSlider :images="product.media" class="col-span-4 md:col-span-1 py-8"></ProductSlider>
      <div class="flex flex-col col-span-4 md:col-span-2 md:me-4">
        <div class="relative">
          <h1 class="font-bold p-4 ml-12 text-xl">
            {{ product.name }}
          </h1>
          <svg @click="toggleFavorite" v-if="customerProduct !== null && customerProduct.is_favorite"
               class="absolute end-4 top-5 product-detail__favorite cursor-pointer" width="24"
               height="24" viewBox="0 0 512 512" fill="none"
          ...
          <svg @click="toggleFavorite" v-else class="absolute end-4 top-5 product-detail__not-favorite cursor-pointer"
               width="24" height="24"
               viewBox="0 0 512 512"
               xmlns="http://www.w3.org/2000/svg">
            <path
        ...
        <div v-if="isPurchasable" class="flex self-end md:self-start md:ps-4 me-4">
          <div class="product-detail__final-price mt-2 me-1">{{ product.price }}</div>
          <svg class="product-detail__final-price-tooman" width="14" height="23" viewBox="0 0 14 23" fill="none"
               xmlns="http://www.w3.org/2000/svg">
            <path
        ...
        <div v-if="isPurchasable  && product.sizes.length !== 0" class="p-4 flex flex-col md:flex-row md:items-center">
          <h2 class="text-lg font-bold">سایز</h2>
          <div class="flex space-x-4 rtl:space-x-reverse mt-2 md:mt-0 md:ms-8">
            <div v-for="(size, index) in product.sizes"
                 class="rounded-full border-2 w-12 h-12 text-lg font-bold flex items-center justify-center">
              <span>{{ size.name }}</span>
            </div>
          </div>
        </div>
        <div v-if="isPurchasable  && product.sizes.length !== 0" class="divider-h md:hidden mt-2"></div>

        <div v-if="isPurchasable && product.colors.length !== 0" class="p-4 flex flex-col md:flex-row md:items-center">
        ...
        <div v-if="isPurchasable && product.colors.length !== 0" class="divider-h md:hidden mt-2"></div>

        <div class="p-4 mt-2 md:hidden">
          <a class="flex justify-between" href="#comment-modal">
            <h2 class="text-lg font-bold">مشخصات</h2>
            <font-awesome-icon class="text-gray-500" icon="chevron-left"></font-awesome-icon>
          </a>
          <div id="comment-modal" class="modal">
            <div class="modal-box h-full w-full rounded-none overflow-y-scroll">
              <div class="flex items-center">
                <a href="#" class="flex items-center">
                  <font-awesome-icon class="text-black text-xl" icon="arrow-right"></font-awesome-icon>
                </a>
      ...
      <div v-if="isPurchasable">
        <div class="flex flex-col text-sm hidden md:block md:col-span-1 rounded-xl bg-gray-100 px-4 py-6">
          <div class="flex justify-between">
            <div>فروشنده</div>
            <a href="#" class="text-xs hidden">4 فروشنده دیگر</a>
          </div>
          <div class="mt-8 flex items-center">
            <font-awesome-icon class="text-gray-800 text-base" icon="store"></font-awesome-icon>
            <div class="text-sm font-bold ms-4">{{ product.sellers[0].shop_name }}</div>
          </div>
          <div v-if="selectedShopProduct.variations[2]" class="md:border-b hidden md:block my-4 w-full"></div>
          <div v-if="selectedShopProduct.variations[2]" class="flex items-center">
            <font-awesome-icon class="text-gray-800 text-base" icon="check"></font-awesome-icon>
            <div class="text-sm font-bold ms-4">گارانتی {{ selectedShopProduct.variations[2].value }}</div>
          </div>
          <div v-if="selectedShopProduct.variations[1]" class="md:border-b hidden md:block my-4 w-full"></div>
          <div v-if="selectedShopProduct.variations[1]" class="flex items-center">
            <font-awesome-icon class="text-gray-800 text-base" icon="check"></font-awesome-icon>
            <div class="text-sm font-bold ms-4">سایز: {{ selectedShopProduct.variations[1].value }}</div>
          </div>
          <div v-if="selectedShopProduct.variations[0]" class="md:border-b hidden md:block my-4 w-full"></div>
          <div v-if="selectedShopProduct.variations[0]" class="flex items-center">
            <font-awesome-icon class="text-gray-800 text-base" icon="pen"></font-awesome-icon>
            <div class="text-sm font-bold ms-4">رنگ:<span> {{ selectedShopProduct.variations[0].value }}</span></div>
          </div>
          <button class="btn w-full border-2 mt-8 btn-primary" @click="addToCart">افزودن به سبد خرید</button>
        </div>
      </div>
    </div>

    <div v-if="isPurchasable" class="container w-full p-4 md:p-0 md:mt-12 text-sm hidden md:block">
      <h3 class="text-base font-bold mb-8">لیست فروشندگان</h3>
      <div v-for="seller in product.sellers" class="flex items-center justify-between rounded-lg px-4">
        <div class="flex items-center">
        ...
        <button class="btn border-2 btn-outline btn-primary" @click="addToCart">افزودن به سبد خرید</button>
      </div>
    ...
    <div class="container mb-4 mt-8 md:mt-12">
      <ProductHorizontalList title="محصولات مشابه" :products="product.related_products"
                             class="mt-8 md:-ms-4"></ProductHorizontalList>
    </div>
    <div class="container z-10  sticky top-0 mb-8 mt-8 hidden md:block">
      <div class="bg-white w-full font-bold text-sm text-gray-500"
    ...
    <div class="container w-full mt-8 hidden md:block">
      ...
      <div class="flex">
        ...
        <div class="flex-1">
          <EmptyState v-if="!productComments.length" :empty-list="emptyList" class="flex h-auto"></EmptyState>
          <CommentList :pagination="productCommentsPagination" :comments="productComments" class="ms-8"></CommentList>
        </div>
      </div>
      <div class="md:border-b hidden md:block mt-8 w-full"></div>
    </div>
...
export default {
  name: "product",
  components: {ProductAttributes, CommentList, CommentsTotalStat, ProductHorizontalList, ProductSlider, EmptyState},
  auth: false,
  data() {
    return {
      view: {
  ...
  async asyncData({$axios, route}) {
    ...
    const productAttribute = await $axios.get(`/products/${productId}/attributes`)
    const productComments = await $axios.get(`customer/products/${productId}/comments`)
    return {
  ...
  methods: {
    ...
    toggleFavorite() {
      if (this.customerProduct.is_favorite) {
        this.$axios.delete(`/customer/user/profile/favorites/${this.product.id}`).then(() => {
          this.customerProduct.is_favorite = false
        })
      } else {
        this.$axios.post(`/customer/user/profile/favorites/${this.product.id}`).then(() => {
